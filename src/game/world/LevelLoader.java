...
public class LevelLoader {
		...
		try {
			...
			for (Object layer : layers) {
				if (((JSONObject) layer).get("objects") != null) {
					...
					/*
					 * for(Object num : data) { JSONObject numObject = (JSONObject)num;
	...
	public static void LoadLevelHeightMap(Handler handler) {
		 ...
		 * for(int y = 0;y<osn.length;y++) { for(int x = 0;x<osn[y].length;x++) { float
		 * val = osn[x][y]; if(val < -0.2) { handler.addObjectNoTick(new Tile(x*16,
		 * y*16, ID.Tile, Textures.tileSetBlocks.get(27))); }else if(val < 0 && val >
		 * -0.2){ handler.addObjectNoTick(new Tile(x*16, y*16, ID.Tile,
		 * Textures.tileSetBlocks.get(7))); }else if(val > 0.5 && val < 0.9){
		 * handler.addObjectNoTick(new Tile(x*16, y*16, ID.Tile,
		 * Textures.tileSetBlocks.get(33))); }else if(val > 0.9) {
		 * handler.addObjectNoTick(new Tile(x*16, y*16, ID.Tile,
		 * Textures.tileSetBlocks.get(34))); } else { handler.addObjectNoTick(new
		 * Tile(x*16, y*16, ID.Tile, Textures.tileSetBlocks.get(18))); }
		 * 
		 * } }
		 */
